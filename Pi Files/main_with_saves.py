...
def launch_game(selection : int, save_selection : int = 0) -> None:
    ...
    if selection == 0:
        ...
        game = Connect4(LEDstrip, disp, img, draw, font, font2, uart, save_selection)
    elif selection == 2:
...
def has_saves(selection : int) -> bool:
    if selection == 1:
        json_files = [pos_json for pos_json in os.listdir("/home/b1128c/BetterBoardGameBoard/Pi Files/saves") if pos_json.endswith('.json') and pos_json.startswith("Connect4")]
        json_files = json_files[::-1]

        if json_files:
            global game_selection
            game_selection = selection
            save_display(json_files)
            return True

    return False

def save_display(saves : list) -> None:
    # resets menu data with menu size of the games
    
    if len(saves) > 5: # only allow up to 5 saves shown
        reset_menu_data(5, 2)
    else:
    ...
    draw.rectangle((13, 47, disp.width - 13, 59), fill = RED)
    text2 = "NEW GAME"
    draw.text((disp.width * (1/2) - (font.getlength(text2) / 2), 50), text2, font = font, fill = WHITE)
    for i in range(0, menu_size + 1): # displays test for every game
        draw.text((disp.width * (1/2) - (font.getlength(saves[i]) / 2), 70 + (20 * i)), saves[i], font = font, fill = WHITE)
    disp.display(img)
